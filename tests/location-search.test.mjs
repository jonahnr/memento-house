...
test('landmarks and international city searches retain original provider coverage',async()=>{
 for(const q of ['Eiffel Tower','London','Statham','10 Downing Street, London SW1A 2AA']){assert.equal(isUSStreetQuery(q),false);const result=await searchPlaces(q,async url=>{assert.ok(url.includes('photon'));return Response.json(photon)});assert.equal(result.places.length,7)}
