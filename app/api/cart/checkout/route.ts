...
export async function POST(request:Request){
 ...
 try{
  ...
  for(const[index,item]of items.entries()){
   ...
   body.set(`metadata[cart_item_${index}]`,JSON.stringify({id:item.id,product:item.product,tier:item.tier,mapType:item.mapType,mapOccasion:item.mapOccasion,addon:item.addon,customizationId}));
  }
