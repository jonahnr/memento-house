...
export async function POST(request:Request){
 ...
 const target=decision==="approved"?"in_production":decision==="changes_requested"?"proof_ready":"awaiting_customer_approval";
 ...
 if(target!==order.order_status)await admin.from("orders").update({order_status:target,updated_at:now,...(decision==="approved"?{approved_at:now}:{})}).eq("id",order.id);
