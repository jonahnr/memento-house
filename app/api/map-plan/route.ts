...
export async function GET(request:Request){
 ...
 let{data:wedding,error:weddingError}=await admin.from("weddings").select("id,owner_user_id,map_type").eq("slug",slug).eq("status","active").maybeSingle() as unknown as {data:{id:string;owner_user_id:string;map_type?:string}|null;error:unknown};
