...
export async function POST(request:Request){
 ...
 if(mediaId){const attached=await admin.from("media_assets").update({timeline_entry_id:created.data.id,updated_at:new Date().toISOString()}).eq("id",mediaId).eq("wedding_id",weddingId).eq("actor_hash",actorHash).eq("media_type","video").is("timeline_entry_id",null).is("deleted_at",null).select("id").single();if(attached.error||!attached.data){await admin.from("timeline_entries").delete().eq("id",created.data.id);return Response.json({error:"The video could not be attached. Your memory was not saved; please try again."},{status:409})}}
 ...
 if(photo instanceof File&&photo.size){
  ...
  if(!uploaded.error){const publicUrl=admin.storage.from(bucket).getPublicUrl(path).data.publicUrl,photoRow=await admin.from("timeline_photos").insert({entry_id:created.data.id,storage_path:path,public_url:publicUrl,alt_text:`Memory shared by ${guest}`});photoSaved=!photoRow.error;if(photoSaved)await admin.from("media_assets").insert({wedding_id:weddingId,media_type:"photo",storage_provider:"supabase",status:"ready",timeline_entry_id:created.data.id,storage_path:path,public_url:publicUrl,alt_text:`Memory shared by ${guest}`,file_name:photo.name,mime_type:photo.type,file_size_bytes:photo.size,uploader_kind:"guest",actor_hash:actorHash})}
 }

 const photoRow=await admin.from("timeline_photos").select("public_url,alt_text").eq("entry_id",created.data.id).maybeSingle();
 const media=await admin.from("media_assets").select("id,media_type,status,public_url,playback_url,thumbnail_url,cloudflare_uid,alt_text,duration_seconds,error_message").eq("timeline_entry_id",created.data.id).is("deleted_at",null);
 return Response.json({entry:{id:created.data.id,date_value:date,approximate_date_label:null,sort_date:date,title,category:"Guest Memory",story,contributor_name:guest,photo:photoRow.data||null,media:media.data||[],destination:{location_name:place,latitude:lat,longitude:lng}},photoSaved});
}
