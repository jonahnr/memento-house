...
export async function POST(request:Request){
 const{url,serviceRoleKey:key}=supabaseServerConfig();if(!key)return Response.json({error:"Like service is unavailable."},{status:503});
 ...
 const owner=await admin.auth.admin.getUserById(wedding.owner_user_id),tier=await resolveMapTier(admin,wedding.owner_user_id,owner.data.user?.user_metadata,owner.data.user?.email,destination.wedding_id);if(!tierIncludesPlus(tier))return Response.json({error:"Hearts are available on Memento Map Plus."},{status:403});
