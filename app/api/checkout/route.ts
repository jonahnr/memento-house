...
export async function POST(request:Request){
 ...
 const response=await fetch("https://api.stripe.com/v1/checkout/sessions",{method:"POST",headers:{Authorization:`Bearer ${key}`,"Content-Type":"application/x-www-form-urlencoded"},body});if(!response.ok)return new Response("Secure checkout could not be started.",{status:502});const session=await response.json() as {url:string};return Response.redirect(session.url,303)
}
