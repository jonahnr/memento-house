...
import {parseMementoMapType,parseMapOccasion} from "../../../lib/memento-map-types";
...
export async function POST(request:Request){
 ...
 const form=await request.formData(),product=String(form.get("product")||""),tier=String(form.get("tier")||""),rawMapType=String(form.get("mapType")||""),mapType=product==="map"?parseMementoMapType(rawMapType):undefined,mapOccasion=parseMapOccasion(form.get("mapOccasion"),mapType),addon=String(form.get("addon")||"none"),addons=addon==="none"?[]:[addon];let item;
 ...
 const body=new URLSearchParams({mode:"payment",success_url:`${origin}/order/success?session_id={CHECKOUT_SESSION_ID}`,cancel_url:`${origin}/order?product=${encodeURIComponent(product)}&tier=${encodeURIComponent(tier)}${mapType?`&type=${encodeURIComponent(mapType)}${mapOccasion?`&occasion=${mapOccasion}`:""}`:""}`,customer_creation:"always",customer_email:identity.user.email||"","client_reference_id":identity.user.id,"line_items[0][quantity]":"1","billing_address_collection":"required",allow_promotion_codes:"true","metadata[catalog_key]":item.key,"metadata[product]":item.productId,"metadata[tier]":item.tierId,"metadata[map_type]":mapType||"","metadata[map_occasion]":mapOccasion||"","metadata[addons]":addons.join(","),"metadata[customization_id]":customizationId,"metadata[customer_user_id]":identity.user.id});
