...
export async function POST(request:Request){
 ...
 let response:Response;try{response=await fetch("https://api.stripe.com/v1/checkout/sessions",{method:"POST",headers:{Authorization:`Bearer ${key}`,"Content-Type":"application/x-www-form-urlencoded"},body,signal:AbortSignal.timeout(12000)})}catch{return Response.json({error:"Stripe did not respond in time. Please try again; no charge was created."},{status:504})}if(!response.ok){const failure=await response.json().catch(()=>null) as {error?:{message?:string}}|null;return Response.json({error:failure?.error?.message||"Secure checkout could not be started."},{status:502})}const session=await response.json() as {url:string};return Response.json({url:session.url},{headers:{"Cache-Control":"no-store"}})
}
