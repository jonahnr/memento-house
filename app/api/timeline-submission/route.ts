...
export async function POST(request:Request){
 ...
 const owner=await admin.auth.admin.getUserById(wedding.owner_user_id);if(owner.data.user?.user_metadata?.product_tier!=="timeline-plus")return Response.json({error:"Guest timeline contributions are not enabled."},{status:403});
