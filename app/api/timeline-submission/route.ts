...
export async function POST(request:Request){
 ...
 const owner=await admin.auth.admin.getUserById(wedding.owner_user_id),tier=await resolveMapTier(admin,wedding.owner_user_id,owner.data.user?.user_metadata,owner.data.user?.email);if(tier!=="timeline-plus")return Response.json({error:"Guest timeline contributions are not enabled."},{status:403});
