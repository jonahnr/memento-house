...
export async function POST(request:Request){
 ...
 try{
  ...
  const current=await context.admin.from("orders").select("id,order_status,customer_email,catalog_key").eq("id",orderId).single();if(current.error)throw current.error;
  const allowed=["ready_for_design","design_in_progress","proof_ready","awaiting_customer_approval","changes_requested"];if(!allowed.includes(current.data.order_status))throw new Error(`Prepare the order for design before sending a proof. Current status: ${current.data.order_status}.`);
  ...
  const upload=await context.admin.storage.from(bucket).upload(path,file,{contentType:file.type||"application/octet-stream"});if(upload.error)throw upload.error;console.info("[admin/proofs] storage upload complete",{orderId,version,path});
  ...
  await context.admin.from("orders").update({order_status:"awaiting_customer_approval",proof_ready_at:now,updated_at:now}).eq("id",orderId);
  ...
  const delivery=await sendProofEmail({recipient:current.data.customer_email,orderId,productName:current.data.catalog_key||"keepsake"});await context.admin.from("order_events").insert({order_id:orderId,event_type:delivery.sent?"proof_email_sent":"proof_email_failed",actor_type:"system",metadata:delivery});console.info("[admin/proofs] proof workflow complete",{orderId,proofId:proof.data.id,emailSent:delivery.sent});return Response.json({ok:true,proof:proof.data,email:delivery,notice:delivery.sent?"Proof uploaded and customer email sent.":"Proof uploaded. The customer email could not be sent; retry the notification separately."});
 }catch(error){console.error("[admin/proofs] upload failed",error);return Response.json({error:error instanceof Error?error.message:"Proof upload failed"},{status:400})}
}
