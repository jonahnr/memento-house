...
export async function POST(request:Request){
 ...
 try{
  ...
  const delivery=await sendProofEmail({recipient:current.data.customer_email,orderId,productName:current.data.catalog_key||"keepsake"});await context.admin.from("order_events").insert({order_id:orderId,event_type:delivery.sent?"proof_email_sent":"proof_email_failed",actor_type:"system",metadata:delivery});return Response.json({proof:proof.data,email:delivery});
