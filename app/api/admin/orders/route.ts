...
export async function GET(request:Request){
  ...
  const byProduct=Object.values(active.reduce((memo:any,order:any)=>{const key=order.catalog_key||`${order.product}:${order.tier}`;memo[key]||={key,orders:0,revenue:0};memo[key].orders++;if(!order.is_test)memo[key].revenue+=Number(order.amount_total||0);return memo},{})),now=Date.now(),since=(days:number)=>active.filter((order:any)=>new Date(order.created_at).getTime()>=now-days*86400000).length,failedEmails=active.reduce((sum:number,order:any)=>sum+(order.events||[]).filter((event:any)=>String(event.event_type).includes("email_failed")).length,0),failedPayments=active.filter((order:any)=>["failed","payment_failed"].includes(String(order.payment_status))).length;
  if(id)return orders[0]?Response.json({order:orders[0],catalog:catalogPublic()}):Response.json({error:"Order not found"},{status:404});
  const supabase=supabaseServerConfig(),stripeSecret=process.env.STRIPE_SECRET_KEY||process.env.STRIPE_LIVE_SECRET_KEY,stripeWebhook=process.env.STRIPE_WEBHOOK_SECRET||process.env.STRIPE_TEST_WEBHOOK_SECRET,healthHistory=await context.admin.from("system_health_runs").select("id,status,checks,source,finished_at,created_at").order("created_at",{ascending:false}).limit(12);
  return Response.json({orders,catalog:catalogPublic(),analytics:{orders:active.length,ordersToday:since(1),ordersWeek:since(7),ordersMonth:since(30),paid:paid.length,revenue,averageOrder:realPaid.length?Math.round(revenue/realPaid.length):0,awaitingApproval:active.filter((order:any)=>order.order_status==="awaiting_customer_approval").length,toShip:active.filter((order:any)=>order.order_status==="ready_to_ship").length,needsAttention:active.filter((order:any)=>["awaiting_customer_information","changes_requested","proof_ready"].includes(order.order_status)).length,failedEmails,failedPayments,byProduct},health:{stripeWebhook:Boolean(stripeWebhook),supabase:Boolean(supabase.url&&supabase.serviceRoleKey),resend:Boolean(process.env.RESEND_API_KEY),missing:[!supabase.serviceRoleKey&&"SUPABASE_SERVICE_ROLE_KEY",!stripeSecret&&"STRIPE_SECRET_KEY",!stripeWebhook&&"STRIPE_WEBHOOK_SECRET",!process.env.RESEND_API_KEY&&"RESEND_API_KEY"].filter(Boolean),history:healthHistory.data||[],historyError:healthHistory.error?.code==="42P01"?"Run migration 010 to activate proactive checks.":healthHistory.error?.message||null}});
}
