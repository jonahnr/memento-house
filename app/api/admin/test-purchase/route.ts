...
export async function POST(request:Request){
 ...
 try{
  ...
  const recipient=String(body.customerEmail||context.user.email).toLowerCase(),result=await fulfillPurchase(context.admin,{source:"admin",sourceId,email:recipient,name:String(body.customerName||"Admin test customer"),userId:body.customerEmail?null:context.user.id,product,tier,addons,amount:item.price,currency:"usd",customizationId}),testEmail=await Promise.race([sendTestOrderEmail({recipient,orderId:result.order.id,productName:item.displayName}),new Promise<{sent:false;error:string}>(resolve=>setTimeout(()=>resolve({sent:false,error:"Confirmation email timed out; the test order was still created."}),5000))]);
