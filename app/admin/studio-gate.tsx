...
export default function StudioGate({title,description,src}:{title:string;description:string;src:string}){
  ...
  const frame=useRef<HTMLIFrameElement>(null),[customization,setCustomization]=useState<any>(),[frameReady,setFrameReady]=useState(false);
  ...
  useEffect(()=>{if(!frameReady||!customization?.payload)return;const send=()=>frame.current?.contentWindow?.postMessage({type:"load-order-customization",payload:customization.payload},location.origin);send();const retry=window.setTimeout(send,300);return()=>window.clearTimeout(retry)},[frameReady,customization]);
