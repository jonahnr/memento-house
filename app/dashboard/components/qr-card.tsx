...
export function QR({wedding,mapUrl,tier}:{wedding:{partner_one_name:string;partner_two_name:string;title:string};mapUrl:string;tier:string}){
 ...
 useEffect(()=>{let active=true;setQr("");setError("");(async()=>{
  ...
  const logo=new Image();logo.src="/brand/memento-house-logo.webp";await logo.decode();
  const ctx=canvas.getContext("2d")!;
  // High error correction and a small central mark preserve the finder patterns and quiet zone.
  const plate=160,start=(canvas.width-plate)/2;ctx.fillStyle="#ffffff";ctx.fillRect(start,start,plate,plate);
  const scale=Math.min(136/logo.naturalWidth,136/logo.naturalHeight),width=logo.naturalWidth*scale,height=logo.naturalHeight*scale;
  ctx.drawImage(logo,(canvas.width-width)/2,(canvas.height-height)/2,width,height);
 ...
 const card=(copy:number)=><div className="qrCard" key={copy}><div className="eyebrow">{names}</div><h2>Help Build<br/><i>{wedding.title||"Our Adventure Map"}</i></h2><p>{tier==="timeline-plus"?"Scan to share where you came from, recommend a place for us to visit, or add a memory to our story.":"Scan to share where you came from or recommend a place for us to visit."}</p>{qr?<img className="realQr" src={qr} alt={`Scannable QR code with the Memento House logo for ${names}'s map`}/>:<p role="status">{error||"Generating QR code…"}</p>}<b>{mapUrl.replace(/^https?:\/\//,"")}</b><small>THANK YOU FOR BEING PART OF OUR STORY</small></div>;
