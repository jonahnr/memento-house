...
export function AccountShell({email,mapEnabled=false,children}:{email?:string;mapEnabled?:boolean;children:ReactNode}){
 ...
 return <main className="accountShell">
  <header className="accountShellHeader"><a href="/" className="houseBrand"><img src="/brand/memento-house-logo.webp" alt="Memento House"/><div>Memento House<small>Your private account</small></div></a><div><CartLink/><span>{email||"Customer account"}</span>{isAdmin&&<a className="button light" href="/admin/orders">Switch to admin view</a>}<button type="button" className="button light" onClick={signOut}>Log out</button></div></header>
