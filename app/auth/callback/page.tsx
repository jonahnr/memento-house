...
export default function AuthCallback(){
 ...
 useEffect(()=>{void(async()=>{
  ...
  const client=getSupabaseBrowserClient(),url=new URL(location.href),code=url.searchParams.get("code"),next=url.searchParams.get("next")||"/account",confirm=url.searchParams.get("confirm")==="1",authError=url.searchParams.get("error_description")||new URLSearchParams(location.hash.slice(1)).get("error_description");
  ...
  if(confirm){const response=await fetch("/api/auth/confirm",{method:"POST",headers:{authorization:`Bearer ${session.data.session.access_token}`}}),result=await response.json().catch(()=>({}));if(!response.ok){await client.auth.signOut({scope:"local"});setMessage(result.error||"Your email was verified, but your account could not be activated. Request a new confirmation email.");return}}
