...
await expect(page.locator('.storyVideoSlot')).toHaveCount(1);await expect(page.locator('.storyVideoSlot').getByRole('button',{name:/play video/i})).toBeVisible();await expect(page.locator('.storyVideoSlot').locator('.timelineLogoFallback')).toHaveCount(0);
...
if(width<=430){const mobileCard=page.locator('.storyTimeline li').nth(3),date=mobileCard.locator('.storyMobileDate'),photo=mobileCard.locator('.storyPhoto'),location=mobileCard.locator('.storyLocationCompact');await expect(date).toBeVisible();await expect(location).toHaveText('Cincinnati, OH');const geometry=await Promise.all([date.boundingBox(),photo.boundingBox()]);expect(geometry[0].x).toBeGreaterThanOrEqual(geometry[1].x+geometry[1].width-1)}
