...
export async function fulfillCartCheckout(admin:SupabaseClient,session:any){
 ...
 for(const[index,item]of items.entries()){
  ...
  const result=await fulfillPurchase(admin,{source:"stripe",sourceId,stripeSessionId:index===0?session.id:null,email,userId,name:session.customer_details?.name||"",product:item.product,tier:item.tier,mapType:item.mapType,mapOccasion:item.mapOccasion,addons,amount:item.amount,currency:session.currency,customizationId:item.customizationId||null,paymentIntentId:session.payment_intent,isTest:session.livemode===false});
