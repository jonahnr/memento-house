...
export type CartItem={id:string;product:string;tier:string;addon:string;customization:string};
export function validateCart(value:unknown):CartItem[]{
 ...
 return value.map(raw=>{
  ...
  const {id,product,tier,addon="none",customization=""}=raw;
  ...
  if(product==="map"&&++maps>1)throw new Error("Choose one Memento Map plan per cart.");
  ...
  return{id,product,tier,addon,customization};
 });
}
